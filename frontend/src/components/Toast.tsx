...
const ACCENT: Record<ToastKind, string> = {
  loading: 'text-teal',
  success: 'text-compliant',
  error: 'text-blocked',
  info: 'text-violet-soft',
};
...
function ToastRow({ item, onDismiss }: { item: ToastItem; onDismiss: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="glass-strong pointer-events-auto rounded-2xl p-4"
    >
      <div className="flex items-start gap-3">
        <Icon
          size={18}
          className={`mt-0.5 shrink-0 ${ACCENT[item.kind]} ${item.kind === 'loading' ? 'animate-spin' : ''}`}
        />
        <div className="min-w-0 flex-1">
          <p className="font-body text-sm text-mist">{item.message}</p>
          {item.hash && (
            <a
              href={`${EXPLORER}/tx/${item.hash}`}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-1 flex items-center gap-1 font-mono text-xs text-haze hover:text-teal"
            >
        ...
        <button
          ...
          onClick={onDismiss}
          className="focus-ring shrink-0 text-faint hover:text-mist"
        >
          <X size={16} />
        </button>
      </div>
