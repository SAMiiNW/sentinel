...
export type TxKind = 'publish' | 'check';
...
export function useTransaction(onConfirmed?: () => void) {
  ...
  const run = useCallback(
    async (
      ...
      try {
        ...
        let result: Policy | null = null;
        for (let i = 0; i < 5; i++) {
          try {
            const stats = await fetchStats();
            const page = await fetchPolicies(
              Math.max(0, Math.floor(Math.max(0, stats.policies - 1) / 20) * 20),
            );
            if (targetId) {
  ...
  const submitPublish = useCallback(
    (address: `0x${string}`, title: string, policy: string, onFlight?: (v: boolean) => void) =>
      run('publish', address, (c) => publishPolicy(c, title, policy), null, onFlight),
    [run],
  );

  const submitCheck = useCallback(
    (address: `0x${string}`, policyId: string, content: string, onFlight?: (v: boolean) => void) =>
      run('check', address, (c) => submitContent(c, policyId, content), policyId, onFlight),
    [run],
  );

  return { state, submitPublish, submitCheck, reset };
}
